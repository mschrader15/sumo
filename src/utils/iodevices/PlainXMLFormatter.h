...
public:
     ...
     */
    bool writeXMLHeader(StreamDeviceType& into, const std::string& rootElement,
     ...
     * @param[in] rootElement The root element to use
     */
    bool writeHeader(StreamDeviceType& into, const SumoXMLTag& rootElement);
     ...
     */
    void openTag(StreamDeviceType& into, const std::string& xmlElement);
     ...
     */
    void openTag(StreamDeviceType& into, const SumoXMLTag& xmlElement);
     ...
     */
    bool closeTag(StreamDeviceType& into, const std::string& comment = "");
     ...
     */
    void writePreformattedTag(StreamDeviceType& into, const std::string& val);

    /** @brief writes arbitrary padding
     */
    void writePadding(StreamDeviceType& into, const std::string& val);
     ...
     */
    template <class T>
    static void writeAttr(StreamDeviceType& into, const std::string& attr, const T& val) {
        into << " " << attr << "=\"" << toString(val, getStreamDevicePrecision(into)) << "\"";
    }
     ...
     */
    template <class T>
    static void writeAttr(StreamDeviceType& into, const SumoXMLAttr attr, const T& val) {
        into << " " << toString(attr) << "=\"" << toString(val, getStreamDevicePrecision(into)) << "\"";
    }
...
// ===========================================================================
template <>
inline void PlainXMLFormatter::writeAttr(StreamDeviceType& into, const SumoXMLAttr attr, const double& val) {
#ifdef HAVE_FMT
    if (auto stream = std::get_if<std::unique_ptr<OStreamDevice>>(&into); stream != nullptr) {
        fmt::print(static_cast<std::ostream&>(**stream), " {}=\"{:.{}f}\"", toString(attr), val, (*stream)->precision());
    }
    else {
...
template <>
inline void PlainXMLFormatter::writeAttr(StreamDeviceType& into, const SumoXMLAttr attr, const std::string& val) {
    into << " " << toString(attr) << "=\"" << val << "\"";
}
