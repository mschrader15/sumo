...
OutputDevice_File::OutputDevice_File(const std::string& fullName, const bool compressed)
    ...
    if (fullName == "/dev/null") {
        myAmNull = true;
#ifdef WIN32
        myFileStream = std::make_unique<StreamDeviceType_>(new std::ofstream("NUL"));
    ...
    if (compressed) {
        try {
            myStreamDevice = std::make_unique<StreamDeviceType_>(new zstr::ofstream(localName.c_str(), std::ios_base::out));
        }
        ...
        myStreamDevice = std::make_unique<StreamDeviceType_>(new std::ofstream(localName.c_str(), std::ios_base::out));
    }
#else
    UNUSED_PARAMETER(compressed);
    myFileStream = std::make_unique<OStreamDevice>(new std::ofstream(localName.c_str(), std::ios_base::out));
#endif
    if (!getStreamDevice<StreamDeviceType_>(myStreamDevice).good()) {
